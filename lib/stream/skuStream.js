...
SkuStream.prototype._read = function () {
	...
	if (this._skuValidator.isValidSku(permutation)) {
		this._pushValid(self._currentIndex, self._currentIndex % 25000 == 0);
	}
	else {
		this._readAgain(self._currentIndex % 2000 == 0);
	}
};

SkuStream.prototype._readAgain = function (pushAsImmediate) {
	var self =  this;
	if (pushAsImmediate) {
...
SkuStream.prototype.permutationArrayAtIndex = function (index) {
	...
	for (var i = 0; i < this._skuAttributes.length; i++) {
		...
		var attrValueIndex = Math.floor(index / this._divisors[i]) % skuAttr.values.length;
